...
public function update_live_db_settings($data)
{  
    if($data)
	{
	  $this->db->where('id',1);
	  $this->db->update('db_settings',$data);
	  return true;
	}
}
public function get_live_connection_data(){
	
	$this->db->where('id', 1);
	$query = $this->db->get('db_settings');
...
public function update_local_db_settings($data)
{  
    if($data)
    {
      $this->db->where('id',2);
...
public function get_local_connection_data(){
...
public function console_products(){
	$query = $this->db->get('console_products');
	if($query){
